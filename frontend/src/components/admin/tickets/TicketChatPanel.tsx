...
export default function TicketChatPanel({ ticketId, mutate }: TicketChatPanelProps) {
  const { data, loading, mutate: refetchTicket } = useAsync<{ data: any }>(
    () => (ticketId ? `/admin/tickets/${ticketId}` : null),
    !!ticketId,
    false,
    true,
    1000 * 60
  ...
  return (
    <div className='h-full min-h-[400px] rounded-xl overflow-hidden border border-border'>
      <TicketChatView ticket={data.data} refetch={handleRefetch} showHeader={true} />
    </div>
  )
}
